 ...
 */
public class GameManager extends App implements Subject {
    private Table table;
    ...
    private List<GameManagerObserver> gmObserve = new ArrayList<>();


    /**
     * notify all observers who are in the list by invoking
     * their update() method 
     */
    public void notifi() {
        for (GameManagerObserver g : gmObserve) {
            g.update();
        }
    }
     ...
     * @param g 
     */
    public void attach(GameManagerObserver g) {
        gmObserve.add(g);
    }
     ...
     * @param g
     */
    public void detach(GameManagerObserver g) {
        gmObserve.remove(g);
    }
    ...
    /**
     * Create Memento in which saving the all the balls, the score, 
     * and the time.
     * 
     * @param savedBall
     * @param scoreScreen
    ...
    private Memento createMemento(ArrayList<Ball> savedBall, Score scoreScreen, Clock savedTime) {
        return new Memento(savedBall, scoreScreen, savedTime);
    }

    /**
     * Getting the Memento to get the previous state of the balls, the score, 
     * and the time, and then assign it back to the current balls, the score, and the time.
     * @param m
     */
    private void restore(Memento m) {
        if (m == null) {
            System.out.println("No more previous state.");
            return;
        }
        balls = m.getSavedBalls();
        scoreScreen = m.getSavedScore();
        time = m.getSavedTime();
    }
    ...
    public void buildManager() {
        ...
        undo  = new Button("Undo Shot"); 
        undo.setLayoutX(480.0);
        undo.setLayoutY(8.0);
        scoreScreen = new Score(0, App.gm);
        time = new Clock(0, 0, App.gm);
    ...
    /**
     * restore() method called when undo button is clicked 
     */
    private void undoButtonHandle () {
    ...
    private void draw() {
        ...
        if (this.cue != null && cueActive ) {
            ...
            gc.strokeOval(handle.getCenterX(), handle.getCenterY(), 10.0, 10.0);
        }
        ...
        gc.fillText("Time: " + time.display(), 10.0, 25.0);
        gc.fillText("Score: " + String.valueOf(scoreScreen.getScore()), 180.0, 25.0);
        undoButtonHandle();
        keyBoardDetect();
    ...
    /**
     * detect keyboard input for cheat feature 
     */
    private void keyBoardDetect () {
        paneM.setOnKeyPressed((event) -> {
            String colour = "";
            switch(event.getCode()) {

            // sapphire = blue to avoid conflict with black
            case S:
                colour = "blue";
                break;
            case P:
            ...
            for (Ball b : balls) {
                if (b.getColourString().equals(colour)) {
                    b.setIsActive(false);
                    score ++;
                    scoreScreen.increaseScore();
                }
            }
        });
    }
    ...
    public void tick() {
        ...
        for (Ball ball : balls) {
            ...
            if (ball.isCue() && cueSet) {
                // saving the state before hitting the cue ball
                ArrayList<Ball> savedBall = new ArrayList<Ball>();
                for (Ball b : balls) {
                    savedBall.add(b.clone());
                }
                Clock savedTime = new Clock(time.getMinute(), time.getSecond(), App.gm);
                Score savedScore = new Score(scoreScreen.getScore(), App.gm);

                caretaker.addMemento(createMemento(savedBall, savedScore, savedTime));
    ...
    private void setClickEvents(Pane pane) {
        pane.setOnMousePressed(event -> {
            ...
            cue.setEndX(event.getX() + 30);
            cue.setEndY(event.getY() + 30);

            handle = new Circle(cue.getEndX(), cue.getEndY(), 10.0, Paint.valueOf("pink"));
        });
        ...
        pane.setOnMouseDragged(event -> {
            ...
            handle.setCenterX(cue.getEndX());
            handle.setCenterY(cue.getEndY());
        });
